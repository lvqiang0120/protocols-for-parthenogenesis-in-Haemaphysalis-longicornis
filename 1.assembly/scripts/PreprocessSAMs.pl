...
if    ( uc($extension) eq 'SAM' ) { run_cmd( "$samtools view -@ 64 -bhS $SAM -o $head.bam" ); }
...
run_cmd( "$samtools view -@ 64 -F12 $head.REduced$nodups.bam -b -o $head.REduced$nodups.paired_only.bam" );
