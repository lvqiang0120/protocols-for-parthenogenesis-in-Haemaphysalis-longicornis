...
sub out_to_gff3 {
        ...
        while (<IN>) {
                ...
                $output .= "$chr\tRepeatMasker\t$class\t$start\t$end\t$score\t$strand\t.\tID=$id;Target=$target $target_start $target_end;Class=$class;PercDiv=$t[1];PercDel=$t[2];PercIns=$t[3];\n";
                $mark++;
        }
        close IN;
