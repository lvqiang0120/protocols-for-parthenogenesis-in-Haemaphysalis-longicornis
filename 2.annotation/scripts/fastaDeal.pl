#!/BJPROJ/GR/DENOVO/USER/leiyang/local/perl/bin/perl
...
use lib '/annoroad/data1/bioinfo/PMO/zhanwei/install/unrar/desti.analysis.ly.anno.dir/pipeline_v2.0/perllib';
use pply;
...
#	my $out_dir = (defined $Outdir) ? "$Outdir/$file_name.cut" : "./$file_name.cut";
	...
	while(<IN>) {
			...
			if ($Cur_len >= $Sub_len) {
				open OUT,">$out_dir/$file_mark.$file_name" || die "fail";
				print OUT $Cur_content;
				close OUT;

				$Cur_len_total+=$Cur_len;
				$Total_len_update=$Total_len-$Cur_len_total;
				$Cutf_update=$Cutf-$file_mark;
				$Sub_len_update=int ($Total_len_update/$Cutf_update)+1;
				$Sub_len=$Sub_len_update;
